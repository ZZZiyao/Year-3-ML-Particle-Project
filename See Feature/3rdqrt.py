...
save_dir = r"D:\Year3\BSc Project\Particle-Machine-Learning\Per_Feature_Plots"
...
importance_df = pd.read_csv(r'D:\Year3\BSc Project\Particle-Machine-Learning\permutation_feature_importance.csv')
...
for feature in features:
    ...
    q3_bg = datasets[0][feature].quantile(0.75)  
    q3_sig = datasets[6][feature].quantile(0.75)  
    x_max = max(q3_bg, q3_sig)  

    sns.histplot(datasets[0][datasets[0][feature] <= x_max][feature], bins=500, kde=False, 
                 color="blue", alpha=0.5, stat="density", label="Background", ax=axes[1])
    sns.histplot(datasets[6][datasets[6][feature] <= x_max][feature], bins=500, kde=False, 
                 color="red", alpha=0.5, stat="density", label="Signal", ax=axes[1])

    axes[1].set_title(f"Histogram of {feature} (Limited to Q3)")
