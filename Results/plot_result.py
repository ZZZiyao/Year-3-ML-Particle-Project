...
thresholds = np.linspace(0.01, 0.99, 100)  #candidate thresholds
f1_scores = []
...
#df.to_csv("f1_score_vs_threshold.csv", index=False)
#print("Saved to f1_score_vs_threshold.csv")
...
#non normalized condusion matrix
print("\nConfusion Matrix:")
print(pd.DataFrame(cm, index=["Actual 0", "Actual 1"], columns=["Predicted 0", "Predicted 1"]))
#%%

prob_0 = y_pred_prob[y_val == 0]  # predictions for real 0 label
prob_1 = y_pred_prob[y_val == 1]  # predictions for real 1 label
